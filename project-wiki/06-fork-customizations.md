...
- `MORE_ITEMS`：**已移除**（⋯ More 菜单不再存在）。原"文件(Sources)"入口并入「文件导入」按钮的弹出菜单
  （命名「**历史文件**」，与导入选项间加分隔线）；质量检查入口移除（有待办时底部 Tasks 自动出现）；Skills 移入设置。
...
dismiss）。点"打开下载页"会同时 `onDismiss()`，故红点与横幅在点击后**立即消失**，直到下一个新版本出现再亮起
（"稍后"按钮同理）。
